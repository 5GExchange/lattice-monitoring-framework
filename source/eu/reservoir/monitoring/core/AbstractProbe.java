...
public abstract class AbstractProbe implements Probe {
    ...
    /*
     * Inform object.
     * This is the last object passed via an inform()
     */
    Object informObject;
	
    /*
     * Is the probe waiting
     */
	...
	name = aName;
	// convert the name into a hashcode and then into an unsigned long
	// WAS myID = new ID(0xffffffffL & getName().hashCode());
	return this;
    }
	...
	Map<String, Object> md = new LinkedHashMap<String, Object> ();
        md.put("id", myID);
        md.put("serviceID", serviceID);
        md.put("groupID", groupID);
	md.put("name", name);
	md.put("dataRate", dataRate);
	md.put("active", threadRunning);
	md.put("on", isOn());
        md.put("informObject", informObject);
    ...
    /**
     * Get the last OnEvent data passed through an inform()
     */
    public Object getLastOnEventData() {
        return informObject;
    }
     ...
     */
    protected boolean checkMeasurement(Measurement meas) {
     ...
     */
    protected ProbeMeasurement collectThenCheck() throws InterruptedException {
	...
	while (threadRunning) {
		...
		try {
		    waitWhileOff();
                    ...
                    // jump out of wait()
		    //System.out.println("AbstractProbe: " + getName() + ": InterruptedException during waitWhileOff()");
		}

                // should we exit
                if (!threadRunning) {
                    //System.out.println("AbstractProbe: " + getName() + ":  !threadRunning after waitWhileOff()");
                    break;
                }
		...
		if (collectiontype ==  Probe.CollectionType.OnEvent) {
		    if ( informsToProcess.decrementAndGet() <= 0 ) {
                        ...
                        turnOffProbe();
			// was setStatus(Probe.Status.OFF);
		    }
		}
		 ...
		 */
		if (m == null) {
		    // null means no useful measurement
		} else {
		    // set probe seq no
    ...
    private void waitFor() {
        ...
        synchronized (this) {
            if (waiting) {
                System.out.println("AbstractProbe:waitFor():  notifyAll() in stopProbeThread() did not work");
                notifyAll();
            }
        }
        */
