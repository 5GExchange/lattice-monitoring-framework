...
public abstract class AbstractDataSource implements DataSource, PlaneInteracter, DataSourceDelegateInteracter, Runnable {
	...
	myID = ID.generate(); // WAS new ID(this.hashCode());
	...
	while (threadRunning) {
	    ...
	    // now send it
            try {
                if (dataSourceDelegate.isConnected()) {
    ...
    /**
     * Callback for when a message is send successfully.
     */
    public void sendSuccess(DataPlaneMessage msg) {
    }

    /**
     * Callback for when a message failed to send
     */
    public void sendFailure(DataPlaneMessage msg, Exception ex) {
